...
CVPR 21: Escaping the Big Data Paradigm with Compact Transformers
"""
...
def cct_7_3x1_32_model(pretrained=False, progress=False,
...
def cct_7_3x1_32_c100_model(pretrained=False, progress=False,
...
class cct_7_3x1_32_c100(fp.Module):

    def __init__(self, device, *args, **kwargs):
        super(cct_7_3x1_32_c100, self).__init__(device)
        self.name = 'cct_7_3x1_32_c100'
        self.model = None
    ...
    def generate_model(self, input_data_shape, target_class_num, *args, **kwargs):
        self.model = cct_7_3x1_32_c100_model(
            num_classes=target_class_num, ignore_head=self.ignore_head).to(self.device)
    ...
    def fix_Loc_list(self):
        ...
        for layer in sd.keys():
            ...
            layer_name = str(layer_name_list[:-1])
            if layer_name not in name_pool:
                name_pool.append(layer_name)
                flag = i
            else:
                temp[flag] = torch.hstack([temp[flag], temp[i]])
                temp[i] = None
            i += 1
        self.Loc_list = []
        for item in temp:
